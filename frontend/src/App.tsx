...
function App() {
  ...
  const handleImportVideoLink = async (url: string) => {
    ...
    if (!trimmedUrl) {
      ...
      setLinkImportError('Enter a TikTok/Douyin video link.');
      return;
    }
  ...
  const handleDeleteImportedVideo = (videoId: string) => {
    ...
    const deletedTimelineClipIds = new Set(
      timelineClips
        .filter((clip) => clip.videoId === videoId)
        .map((clip) => clip.id)
    );
    const isUsedOnTimeline = deletedTimelineClipIds.size > 0;

    if (
      isUsedOnTimeline &&
      !window.confirm(
        `Delete "${video.name}" from Media? This will also remove ${deletedTimelineClipIds.size} timeline clip${deletedTimelineClipIds.size === 1 ? '' : 's'} and related text/audio.`
      )
    ) {
      return;
    }

    recordHistory(isUsedOnTimeline ? 'Deleted media and timeline clips' : 'Deleted media');

    const remainingVideos = videos.filter((currentVideo) => currentVideo.id !== videoId);
    const nextTimelineClips = reflowTimelineClips(
      timelineClips.filter((clip) => !deletedTimelineClipIds.has(clip.id))
    );
    const firstDeletedClipIndex = timelineClips.findIndex((clip) => deletedTimelineClipIds.has(clip.id));
    const nextSelectedTimelineClip =
      firstDeletedClipIndex >= 0
        ? nextTimelineClips[Math.min(firstDeletedClipIndex, nextTimelineClips.length - 1)] ?? null
        : null;
    const deletedTextClipIds = new Set(
      textClips
        .filter((clip) => deletedTimelineClipIds.has(clip.timelineClipId))
        .map((clip) => clip.id)
    );
    const deletedBlurMaskClipIds = new Set(
      blurMaskClips
        .filter((clip) => deletedTimelineClipIds.has(clip.timelineClipId))
        .map((clip) => clip.id)
    );
    const deletedDubbingClipIds = new Set(
      dubbingClips
        .filter((clip) => deletedTimelineClipIds.has(clip.timelineClipId) || deletedTextClipIds.has(clip.textClipId))
        .map((clip) => clip.id)
    );
    ...
    setTextClips((currentClips) =>
      currentClips.filter((clip) => !deletedTimelineClipIds.has(clip.timelineClipId))
    );
    ...
    setDubbingClips((currentClips) =>
      currentClips.flatMap((clip) => {
        if (deletedTimelineClipIds.has(clip.timelineClipId) || deletedTextClipIds.has(clip.textClipId)) {
          return [];
        }

        const shift = clipShiftMap.get(clip.timelineClipId) ?? 0;

        return [shift ? { ...clip, start: clip.start + shift, end: clip.end + shift } : clip];
      })
    );
    setSelectedTimelineClipId((currentClipId) =>
      currentClipId && deletedTimelineClipIds.has(currentClipId)
        ? nextSelectedTimelineClip?.id ?? null
        : currentClipId
    );
    setActiveVideoId((currentVideoId) => {
      ...
      return nextSelectedTimelineClip?.videoId ?? remainingVideos[0]?.id ?? null;
    });
    setSelectedTextClipIds((currentClipIds) =>
      currentClipIds.filter((clipId) => !deletedTextClipIds.has(clipId))
    );
    ...
    setSelectedBlurMaskClipId((currentClipId) =>
      currentClipId && deletedBlurMaskClipIds.has(currentClipId) ? null : currentClipId
    );
    setSelectedDubbingClipId((currentClipId) =>
      currentClipId && deletedDubbingClipIds.has(currentClipId) ? null : currentClipId
    );

    if (selectedTimelineClipId && deletedTimelineClipIds.has(selectedTimelineClipId)) {
      setCurrentTime(0);
      setSeekCommand((currentCommand) => ({
        id: currentCommand.id + 1,
        time: 0,
            ...
            {settingsTab === 'downloader' && (
              <>
                <label className="block text-[11px] font-medium uppercase text-gray-500">
                  Downloader API URL
                  <input
                    value={downloaderSettingsDraft.api_base_url}
                    onChange={(event) =>
                ...
                <div className="rounded border border-[#343434] bg-[#151515] px-3 py-2 text-[11px] leading-relaxed text-gray-400">
                  The app will call your downloader server at /api/parse and /api/download. If the server IP changes, update this URL and save settings.
                </div>
              </>
            )}
      ...
      <div className="flex min-h-[240px] flex-1 overflow-hidden">
        ...
        >
          <Sidebar
            ...
            onDeleteVideo={handleDeleteImportedVideo}
